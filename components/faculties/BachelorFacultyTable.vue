...
const props = defineProps<{
  faculty: Faculty<BachelorApplicant>
}>()

const isColumnsSortable = computed((): boolean => props.faculty.departments.length > 1)

const computeStatistics = (stages: Stage<BachelorApplicant>[]) => {
  let firstStageStatistics
  let secondStageStatistics

  stages.forEach(stage => {
    const passedApplicants = stage.applicants.filter(({ is_passed }) => is_passed)
    const passedApplicantsPrimaryScores = passedApplicants.map(({ primary_score }) => primary_score)
    const primaryScores = stage.applicants.map(({ primary_score }) => primary_score)
...
const departments = computed(() => {
  return props.faculty.departments.map(department => {
    return {
      id: department.id,
      name: `${department.emoji} ${department.name.ru}`,
